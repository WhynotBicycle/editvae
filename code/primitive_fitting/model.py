...
from nets import SegNets, NVPSegNets, KLoss, SymmSegNets
...
class trainPrimitives(nn.Module):
    def __init__(self, train_loader, test_loader, device, regularizer_terms=None, category=None, n_parts=8, NVP=False, beta=0.01, epoch=1000, symm=False):
        super().__init__()
        ...
        if self.symm:
            self.model = SymmSegNets(latent_size=256, n_prims=4, n_trans=self.n_parts).to(self.device)
            self.loss = SymmLoss(self.device).to(self.device)
        elif self.NVP:
            self.model = NVPSegNets(latent_size=256, prim_size=16, n_parts=8, device=self.device).to(self.device)
            self.loss = DualLoss(self.device).to(self.device)
            self.kloss = KLoss()
        else:
            self.model = SegNets(latent_size=256, n_primitive=self.n_parts).to(self.device)
            self.loss = DualLoss(self.device).to(self.device)
        self.optimizer = optim.Adam(self.model.parameters(), lr = 1e-3)
        
        if regularizer_terms is None:
            self.regularizer_terms = {"regularizer_type": ['bernoulli_regularizer', 'parsimony_regularizer'],
...
#         self.md_name = 'test'
        self.md_name = 'primFit_' + (self.category if self.category is not None else '')+\
                       '_beta'+str(self.beta).replace('.','')+\
                       '_epoch'+str(epoch)+\
                       ('_NVP' if self.NVP else '')+\
                       ('_symm' if self.symm else '')+\
                       ('_regBern'+str(self.regularizer_terms['bernoulli_regularizer_weight']).replace('.','')+\
                        '_regEB'+str(self.regularizer_terms['entropy_bernoulli_regularizer_weight']).replace('.','')+\
                        '_regParsimony'+str(self.regularizer_terms['parsimony_regularizer_weight']).replace('.','')+\
                        '_regSparsity'+str(self.regularizer_terms['sparsity_regularizer_weight']).replace('.','')+\
                        '_regOverlap'+str(self.regularizer_terms['overlapping_regularizer_weight']).replace('.','')+\
                        '_min'+str(self.regularizer_terms['minimum_number_of_primitives']).replace('.','')+\
                        '_max'+str(self.regularizer_terms['maximum_number_of_primitives']).replace('.','') if self.regularizer_terms is not None else '')
        print(self.md_name)
    ...
    def run_with_NVP(self):
        for epoch in range(self.epoch):
            for _iter, data in enumerate(self.train_loader):
                ...
                primitive_params, mu, logvar = self.model(point)
                ls, pcl_to_prim, prim_to_pcl, regs, _ = self.loss(point, batched_params(primitive_params), self.sampler, self.regularizer_terms)
                kl = self.kloss(mu, logvar)
                ls = ls + self.beta*kl
                ls.backward()
                self.optimizer.step()
                ...
                print("[Epoch/Iter] ", "{:3} / {:3}".format(epoch, _iter),
                      ...
                      "[ KL Divergence ] ", "{: 7.6f}".format(kl),
                      "[ time ]", "{: 7.6f}".format(t2 - t))

            if epoch%500 == 0:
                torch.save(self.model.state_dict(), self.path+'model/'+self.md_name+'_'+str(epoch+1)+'.pth')
    ...
    def run(self):
        for epoch in range(self.epoch):
            if epoch is 40000:
                ...
                for i in range(5):
                    for p in self.model.decoders[i].parameters():
                        p.requires_grad = False
...
#                 point = point.to(self.device)
                ...
                ls, pcl_to_prim, prim_to_pcl, regs, emb_regs = self.loss(point, primitive_params, self.sampler, self.regularizer_terms)
                ls.backward()
                self.optimizer.step()
                ...
                print("[Epoch/Iter] ", "{:3} / {:3}".format(epoch, _iter),
                      ...
                      "[ primitives 2 point clouds ] ", "{: 7.6f}".format(prim_to_pcl.item()),
                      "[ regularizer ] ", "{: 7.6f}".format(regs.item()),
                      "[ emb reg ] ", "{: 7.6f}".format(emb_regs.item()),
                      "[ time ]", "{: 7.6f}".format(t2 - t))
                
            if epoch in [9999, 19999, 29999, 39999]:
                torch.save(self.model.state_dict(), self.path+'model/'+self.md_name+'_'+str(epoch+1)+'.pth')
        ...
        for _iter, data in enumerate(self.train_loader):
            if _iter == 10:
                ...
                if self.symm:
                    ...
                    for i in [2,3,4]:
                        primitive_params[i] = torch.gather(primitive_params[i], 1, idx.unsqueeze(2).repeat(1,1,primitive_params[i].size(2)))
